...
describe("calculator", () => {
  ...
  //Another test step - test out sub
  it('Subtraction',async () => {
    await program.methods.subtract(new anchor.BN(9), new anchor.BN(3))
    .accounts({
        calculator: calculatorPair.publicKey,
    })
    .rpc()
  ...
  it('Multiplication',async () => {
    await program.methods.multiply(new anchor.BN(9), new anchor.BN(3))
    .accounts({
        calculator: calculatorPair.publicKey,
    })
    .rpc()
    const account = await program.account.calculator.fetch(calculatorPair.publicKey)
    expect(account.result).to.eql(new anchor.BN(18))
  })
